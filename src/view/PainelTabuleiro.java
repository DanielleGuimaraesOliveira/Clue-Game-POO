...
class PainelTabuleiro extends JPanel implements Observador {
    ...
    public PainelTabuleiro(Image imagem, ControladorPartida controlador, JanelaJogo janelaPai) {
        this.imagem = imagem;
    ...
    private void desenharMovimentosValidos(Graphics2D g2d) {
        ...
        List<ICasa> casas = controlador.mapearCasas(valorDados);

        for (ICasa c : casas) {
            int px = OFFSET_X + (c.getX() * TAMANHO_CASA);
            int py = OFFSET_Y + (c.getY() * TAMANHO_CASA);

            boolean isComodo = !c.getTipo().equals("1") && !c.getTipo().equals("p") && !c.getTipo().equals("0");
            
            if (isComodo) {
                g2d.setColor(new java.awt.Color(0, 100, 0, 150));
                g2d.fillRect(px, py, TAMANHO_CASA, TAMANHO_CASA);

                g2d.setColor(new java.awt.Color(0, 80, 0)); 
    ...
    private void desenharPecas(Graphics2D g2d) {
        ...
        int tamanhoPiao = (int) (TAMANHO_CASA * 0.7);
        int margemX = (TAMANHO_CASA - tamanhoPiao) / 2;
        int margemY = (TAMANHO_CASA - tamanhoPiao) / 2;
        ...
        for (IJogador j : controlador.getJogadores()) {
            ...
            int xPixelBase = OFFSET_X + (posicao.getX() * TAMANHO_CASA);
            int yPixelBase = OFFSET_Y + (posicao.getY() * TAMANHO_CASA);
            int xFinal = xPixelBase + margemX;
            int yFinal = yPixelBase + margemY;
